...
class MultiCustomMSE(torch.autograd.Function):
    @staticmethod
...
def BCELoss_prime(conf, label, w=[1.0, 1.0]): # np.exp(-100) is for numerical stability
    if label == 0:
        return max(conf, np.exp(-100))*max(1-conf, np.exp(-100))/max((1-conf), np.exp(-100)) # w[0]/(1-conf)
    if label == 1:
...
def AAI_adversary(adv_model, shadow, positives, negatives, learning_rate, epochs, seed, remove=1):
    ...
    weight = torch.cat((torch.tensor(2*np.ones((x_train.shape[0]-2*N, N))),
                        torch.tensor(20000*np.ones((N, N))),
                        torch.tensor(20000*np.ones((N, N)))))
        
    for e in range(epochs):
        fc2 = adv_model(x_train)
        loss = MultiCustomMSE.apply(fc2, 400*y_train.float(), weight)
        loss.backward() 
    ...
    with torch.no_grad():
        ...
        for k in range(N):
            ...
            fp = n - tn
            print(f'Training ({e} epochs): TNR = {tn/n}, FPR = {fp/n}, avg = {torch.mean(fc2)}')
...
def WADM_eval(N, model_w, global_model, adv_model, alpha, total_neurons, save_path, mitigate=False):
    ...
    adv_w = adv_model.state_dict()
    return tpr, fpr, missed, index, adv_w # evalute attack performance with these adv_w model weights


def realistic_eval(N, data_train, adv_model, global_model, neurons, remove):
    DEVICE = torch.device('cpu')
...
def exp(numneurons, n_input, seed, unknown, lr, shadow, epochs, target, indexes, model, data_train, evaluation='theoretical', remove=1, model_w=None, alpha=None, n_iter=None):
    ...
    #if evaluation == 'realistic': # not tested
    ...
    if evaluation == 'WADM_mitigation':
        ...
        tp, fn, tn, fp = theoretical_eval(N, target, data_train, unknown, adv_model, flip, remove)
        return tp, fn, tn, fp, adv_model
    #if evaluation == 'WADM_mitigation_realistic': # not tested
    #    tp, fp, missed, index, adv_w = WADM_eval(N, model_w, global_model, adv_model, alpha, total_neurons=int(numneurons/2),
    #                                             save_path=save_path, mitigate=True)
    #    adv_model = adv_model_init(numneurons, global_model=global_model, n_target=N, n_input=n_input)
    #    adv_model.load_state_dict(adv_w)
